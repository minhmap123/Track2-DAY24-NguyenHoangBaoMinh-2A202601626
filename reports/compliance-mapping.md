...
| ASI03 — privilege abuse | Policy context tách owner Run A/Run B, phân loại dữ liệu và chặn egress restricted; ledger gắn `agent_id`/`run_id` cho từng action. TTL chưa implement. | `agent/runner.py:92`, `agent/policy.py:39`, `reports/ledger.jsonl` |
